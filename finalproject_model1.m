...
Q1 = -0.045;   % Spring 1 bottom attachment point (m)
...
l1_o = 0.065;    % Rest length of spring 1 (m)
l2_o = 0.02;    % Rest length of spring 2 (m)

% External (applied) torque (function of time)
%tau = @(t) -40 * ((exp(-t))^((exp(t))^49.5));
tau = @(t) 0;

% Damping coefficient
c = 0.1;   % Adjust as needed
...
disp(['Equilibrium for tau_const = -2 (target near theta = 90 deg): ', num2str(theta_eq2), ' rad, ', num2str(rad2deg(theta_eq2)), ' deg']);
